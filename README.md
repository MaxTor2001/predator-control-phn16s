...
## В работе: Dynamic Boost для видеокарты

Не закончено и не проверено. У RTX 5070 Laptop лимит мощности по умолчанию 70 Вт при максимуме 115 Вт;
поднимать его должен демон `nvidia-powerd` (Dynamic Boost). Прошивка сообщает
...
- `sudo nvidia/try-powerd.sh` — включить демон временно, до перезагрузки; `... stop` — отменить. **Ещё не запускался.**
- `tools/gpu_bench.sh` — замер GPU под нагрузкой (Vulkan-шейдер через `wgpu`, без root), останавливается при GPU > 87 °C.

Базовая линия без демона (70 с нагрузки, режим Balanced): P0, 2790 МГц из 3090, 66–67 Вт при лимите 70 Вт,
GPU 68 °C, 9,7 прохода/с. Первые ~20 с драйвер держит карту в P4 (19 Вт) независимо от режима Acer.
